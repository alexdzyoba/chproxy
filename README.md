[![Go Report Card](https://goreportcard.com/badge/github.com/alexdzyoba/chproxy)](https://goreportcard.com/report/github.com/alexdzyoba/chproxy) 
[![Build Status](https://travis-ci.org/alexdzyoba/chproxy.svg?branch=master)](https://travis-ci.org/alexdzyoba/chproxy?branch=master)
[![Coverage](https://img.shields.io/badge/gocover.io-75.7%25-green.svg)](http://gocover.io/github.com/alexdzyoba/chproxy?version=1.9)
...
- Easy to [configure](https://github.com/alexdzyoba/chproxy/blob/master/config/examples/simple.yml):
```yml
server:
  http:
    listen_addr: ":9090"
...
Precompiled `chproxy` binaries are available [here](https://github.com/alexdzyoba/chproxy/releases).
...
```
go get -u github.com/alexdzyoba/chproxy
```
...
The following minimal `chproxy` config may be used for [this use case](https://github.com/alexdzyoba/chproxy/blob/master/config/examples/spread.inserts.yml):
```yml
server:
  http:
      listen_addr: ":9090"
...
The following minimal `chproxy` config may be used for [this use case](https://github.com/alexdzyoba/chproxy/blob/master/config/examples/spread.selects.yml):
```yml
server:
  http:
      listen_addr: ":9090"
...
The following `chproxy` config may be used for [this use case](https://github.com/alexdzyoba/chproxy/blob/master/config/examples/https.yml):
```yml
server:
  https:
    listen_addr: ":443"
...
All the above cases may be combined in a single `chproxy` [config](https://github.com/alexdzyoba/chproxy/blob/master/config/examples/combined.yml):
...
### Server
`Chproxy` may accept requests over `HTTP` and `HTTPS` protocols. [HTTPS](https://github.com/alexdzyoba/chproxy/blob/master/config#https_config) must be configured with custom certificate or with automated [Let's Encrypt](https://letsencrypt.org/) certificates.

Access to `chproxy` can be limitied by list of IPs or IP masks. This option can be applied to [HTTP](https://github.com/alexdzyoba/chproxy/blob/master/config#http_config), [HTTPS](https://github.com/alexdzyoba/chproxy/blob/master/config#https_config), [metrics](https://github.com/alexdzyoba/chproxy/blob/master/config#metrics_config), [user](https://github.com/alexdzyoba/chproxy/blob/master/config#user_config) or [cluster-user](https://github.com/alexdzyoba/chproxy/blob/master/config#cluster_user_config).
...
Requests to `chproxy` must be authorized with credentials from [user_config](https://github.com/alexdzyoba/chproxy/blob/master/config#user_config). Credentials can be passed via [BasicAuth](https://en.wikipedia.org/wiki/Basic_access_authentication) or via `user` and `password` [query string](https://en.wikipedia.org/wiki/Query_string) args.
...
or a list of replicas with nodes. See [cluster-config](https://github.com/alexdzyoba/chproxy/tree/master/config#cluster_config) for details.
...
under `default` user. The user may be overriden with [kill_query_user](https://github.com/alexdzyoba/chproxy/blob/master/config#kill_query_user_config).

If `cluster`'s [users](https://github.com/alexdzyoba/chproxy/blob/master/config#cluster_user_config) section isn't specified, then `default` user is used with no limits.

### Caching

`Chproxy` may be configured to cache responses. It is possible to create multiple
[cache-configs](https://github.com/alexdzyoba/chproxy/blob/master/config/#cache_config) with various settings.
...
### Security
`Chproxy` removes all the query params from input requests (except the user's [params](https://github.com/alexdzyoba/chproxy/blob/master/config#param_groups_config) and listed [here](https://github.com/alexdzyoba/chproxy/blob/master/scope.go#L292))
...
#### Example of [full](https://github.com/alexdzyoba/chproxy/blob/master/config/testdata/full.yml) configuration:
```yml
...
#### Full specification is located [here](https://github.com/alexdzyoba/chproxy/blob/master/config)
...
An example of [Grafana's](https://grafana.com) dashboard for `chproxy` metrics is available [here](https://github.com/alexdzyoba/chproxy/blob/master/chproxy_overview.json)
